...
import { BannerBorder } from "site/sdk/types/banner.ts";
...
/** @title Banner totalmente customizável */
export interface FullBannerProps extends Omit<BannerProps, "hasPadding"> {}
...
export interface SingleBannerProps {
  ...
  banner: SimplifiedBannerProps | FullBannerProps;
}
  ...
  return (
    (device === deviceDisplay || deviceDisplay === "both")
      ? (
        <section
          class={`${
            clx("w-full max-container-auto", padding && padding, mt && mt)
          }`}
        >
          <Banner
            {...banner}
...
export function LoadingFallback() {
  return <BannerSkeleton bannerType="single" />;
}
